...
export class TestUtils {
  ...
  static async selectDirectory(): Promise<void> {
    try {
      const result = await DirectoryAnalyzer.selectDirectory();
      const directoryUri = result.directoryUri;
      await this.listFilesInDirectory(directoryUri);
    } catch(err) {
      console.error('Error selecting directory:', err);
    }
  }

  static async listFilesInDirectory(directoryUri: string): Promise<void> {
    try {
      ...
      // Process the mp3Files as needed
    } catch(err) {
